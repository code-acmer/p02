...
%% -include("define_worlds.hrl").
%% -include("define_protocol.hrl").
%% -include("define_scene.hrl").
%% -include("define_monster.hrl").
%% -include("define_artifact.hrl").
%% -include("define_dungeon.hrl").
%% -include("define_career.hrl").
%% -include("define_player.hrl").
%% -include("define_task.hrl").
%% -include("define_gift.hrl").
%% -include("define_tavern.hrl").
%% -include("define_mrank.hrl").
%% -include("define_partner.hrl").
%% -include("define_rank.hrl").
%% -include("define_offline.hrl").
%% -include("define_common.hrl").
%% -include("define_arena.hrl").
%% -include("define_treasure.hrl").
%% -include("define_combat.hrl").
%% -include("define_beast.hrl").
%% -include("define_fight_soul.hrl").
%% -include("define_fight_report.hrl").
%% -include("define_mystery_shop.hrl").
%% -include("define_star_power.hrl").
%% -include("define_vip.hrl").
%% -include("define_kilo_ride.hrl").
%% -include("define_trial.hrl").
%% -include("define_activity.hrl").
%% -include("define_activity_rate.hrl").
%% -include("define_boss.hrl").
%% -include("define_warcraft.hrl").
%% -include("define_sys_acm.hrl").
%% -include("define_guild.hrl").
%% -include("define_southern.hrl").
%% -include("define_achieve.hrl").
%% -include("define_master_slave.hrl").
%% -include("define_push.hrl").
%% -include("define_gift_code.hrl").
%% -include("define_ups.hrl").
%% -include("define_store_receipt.hrl").
%% -include("define_buff.hrl").
%% -include("define_reward.hrl").
%% -include("define_delayer.hrl").
%% -include("define_sys_setting.hrl").
...
%% init_ets() ->
%%     %% 敏感词表
%%     ets:new(?ETS_WORDS, [named_table, public, set]),

%%     %% 基础协议信息
%%     ets:new(?ETS_BASE_PROTOCOL,
%%             [{keypos, #ets_base_protocol.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基础_怪物信息
%%     ets:new(?ETS_BASE_MON,
%%             [{keypos, #ets_mon.mid}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基础_NPC信息
%%     ets:new(?ETS_BASE_NPC,
%%             [{keypos, #ets_npc.nid}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 法宝相关信息
%%     ets:new(?ETS_BASE_ARTIFACT, 
%%             [{keypos, #ets_base_artifact.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基础_场景信息
%%     ets:new(?ETS_BASE_SCENE,
%%             [{keypos, #ets_scene.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基本_场景坐标表
%%     %% ets:new(?ETS_BASE_SCENE_POSES, [named_table, public, bag]),

%%     %% 基础_场景怪物信息
%%     ets:new(?ETS_BASE_SCENE_MON,
%%             [{keypos, #ets_mon.unique_key}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基础_场景NPC信息
%%     ets:new(?ETS_BASE_SCENE_NPC,
%%             [{keypos, #ets_npc.unique_key}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基础_副本信息
%%     ets:new(?ETS_BASE_DUNGEON,
%%             [{keypos, #dungeon.id}, named_table, public, set,
%%              {read_concurrency, true}]),

%%     %% 基础职业属性
%%     ets:new(?ETS_BASE_CAREER, 
%%             [{keypos, #ets_base_career.career_id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 基础战斗Buff表
%%     ets:new(?ETS_BASE_COMBAT_BUFF,
%%             [{keypos,#rec_base_combat_buff.buff_id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_PLAYER,
%%             [{keypos, #rec_base_player.id}, named_table, public, set,
%%              {read_concurrency, true}]), 
%%     ets:new(?ETS_BASE_GOODS,
%%             [{keypos, #ets_base_goods.goods_id}, named_table, public, set,
%%              {read_concurrency, true}]),
...
%%     ets:new(?ETS_BASE_EQUIP_FORGE,
%%             [{keypos, #ets_base_equip_forge.id}, named_table, public, set,
%%              {read_concurrency, true}]),

%%     %% ets:new(?ETS_BASE_EQUIP_UPGRADE, [{keypos,#ets_base_equip_upgrade.id},named_table,public,set]),


%%     ets:new(?ETS_BASE_GOODS_DROP_NUM,
%%             [{keypos, #ets_base_goods_drop_num.id}, named_table, public, set]),
%%     ets:new(?ETS_BASE_GOODS_DROP_RULE,
%%             [{keypos, #ets_base_goods_drop_rule.id}, named_table, public, set]),
...
%%     ets:new(?ETS_SHOP_GOODS, [{keypos, #ets_shop_goods.id}, named_table, public, set]),
...
%%     ets:new(?ETS_BASE_TASK,
%%             [{keypos, #task.id}, named_table, public, set,
%%              {read_concurrency, true}]),
...
%%     %% 本节点在线用户列表
%%     ets:new(?ETS_ONLINE, [{keypos,#player.id}, named_table, public, set]),
%%     %% 本节点离线用户列表
%%     ets:new(?ETS_OFFLINE,
%%             [{keypos, #player.id}, named_table, public, set]),
%%     %% 玩家配置数据
%%     ets:new(?ETS_PLAYER_SYS_SETTING,
%%             [{keypos, #player_sys_setting.player_id}, named_table, public, set]),
%%     %% 本节点加载场景在线用户列表
%%     %% ets:new(?ETS_ONLINE_SCENE, [{keypos,#player.id}, named_table, public, set]),
%%     %% 本节点场景信息
%%     ets:new(?ETS_SCENE, [{keypos, #ets_scene.sid}, named_table, public, set]),
%%     %% 本节点怪物信息
%%     ets:new(?ETS_SCENE_MON, [{keypos, #ets_mon.unique_key}, named_table, public, set]),
%%     %% 本节点NPC信息
%%     ets:new(?ETS_SCENE_NPC, [{keypos, #ets_npc.unique_key}, named_table, public, set]),
%%     %% 黑名单操作记录表
%%     ets:new(?ETS_BLACKLIST, [{keypos, #ets_blacklist.id}, named_table, public, set]),

%%     %% 本节点在线玩家的背包物品表
%%     %% ets:new(?ETS_GOODS_ONLINE, [{keypos, #goods.id}, named_table, public, set]),
%%     %% 本节点在线玩家的物品属性表
%%     %% ets:new(?ETS_GOODS_ATTRIBUTE, [{keypos, #goods_attribute.id}, named_table, public, set]),
%%     %% 物品掉落表
%%     %% ets:new(?ETS_GOODS_DROP, [{keypos, #ets_goods_drop.id}, named_table, public, set]),
%%     %% 物品buff表
%%     %% ets:new(?ETS_GOODS_BUFF, [{keypos, #goods_buff.id}, named_table, public ,set]),
%%     %% 物品使用cd表 
%%     %% ets:new(?ETS_GOODS_CD, [{keypos,#ets_goods_cd.id}, named_table,public ,set]),

%%     %% 角色任务记录
%%     ets:new(?ETS_ROLE_TASK,
%%             [{keypos, #role_task.id}, named_table, public, set]),
%%     %% 角色任务历史记录
%%     ets:new(?ETS_ROLE_TASK_LOG, [named_table, public, bag]),
%%     %% 当前所有可接任务
%%     %% ets:new(?ETS_TASK_QUERY_CACHE, [named_table, public, set]),
%%     %% 玩家任务缓存
%%     ets:new(?ETS_TASK_CACHE, [named_table, public, set]),
%%     %% 玩家关系记录
%%     ets:new(?ETS_RELA, [{keypos, #ets_rela.id}, named_table, public, set]),
%%     %% 基础祝福配置
%%     %% ets:new(?ETS_BASE_BLESS,[{keypos, #ets_base_bless.id}, named_table, public, set]),
%%     %% 玩家每日的祝福信息
%%     %% ets:new(?ETS_DAILY_BLESS,[{keypos, #ets_daily_bless.player_id}, named_table, public, set]),
%%     %% 玩家延时信息
%%     ets:new(?ETS_DELAYER, [{keypos,#ets_delayer.id}, named_table, public, set]),
%%     %% 招募小黑板
%%     %% ets:new(?ETS_BLACKBOARD, [{keypos,#ets_blackboard.id}, named_table, public, set]),

%%     %% 其实不用每个节点都创建这个ets
%%     %% 组队信息的ETS
%%     %% ets:new(?ETS_TEAM_INFO, [{keypos,#ets_team_info.team_id}, named_table, public, set]),
%%     %% 宠物性格技能
%%     %% ets:new(?ETS_BASE_PET_CHARACTER_SKILL, [{keypos,#ets_base_pet_character_skill.id}, named_table, public, set]),
%%     %% 宠物被动技能
%%     %% ets:new(?ETS_BASE_PET_PASSIVE_SKILL, [{keypos,#ets_base_pet_passive_skill.id}, named_table, public, set]),
%%     %% 宠物主动技能
%%     %% ets:new(?ETS_BASE_PET_ACTIVE_SKILL, [{keypos,#ets_base_pet_active_skill.id}, named_table, public, set]),
...
%%     %% 经脉基础属性
%%     %% ets:new(?ETS_BASE_MERIDIAN, [{keypos, #ets_base_meridian.mer_id}, named_table, public, set]),
%%     %% ets:new(?ETS_BASE_MERIDIAN_CD,[{keypos, #ets_base_meridian_cd.id}, named_table, public, set]),
%%     %% 玩家经脉属性
%%     %% ets:new(?ETS_MERIDIAN, [{keypos, #ets_meridian.id}, named_table, public, set]),
...
%%     %% 在线奖励玩家表
%%     ets:new(?ETS_ONLINE_GIFT,
%%             [{keypos, #ets_online_gift.player_id}, named_table, public, set]),
%%     %% 奖励物品表
%%     ets:new(?ETS_BASE_ONLINE_GIFT,
%%             [{keypos, #ets_base_online_gift.times}, named_table, public,set]),
%%     %% 登录奖励列表
%%     ets:new(?ETS_BASE_LOGIN_GIFT,
%%             [{keypos, #ets_base_login_gift.id}, named_table, public, ordered_set,
%%              {read_concurrency, true}]),
%%     %% 场景分类
%%     %% ets:new(?ETS_BASE_SCENE_CLASS,
%%     %%         [{keypos, #ets_base_scene_class.scene_id}, named_table, public, set]),
%%     %% 阵营玩家统计缓存
%%     %% ets:new(?ETS_REALM, [named_table, public, set]),
%%     %% 目标奖励玩家表
%%     %% ets:new(?ETS_TARGET_GIFT, [{keypos, #ets_target_gift.player_id}, named_table, public, set]),
%%     %% 目标奖励物品表
%%     %% ets:new(?ETS_BASE_TARGET_GIFT,[{keypos, #ets_base_target_gift.day}, named_table, public,set]),
%%     %% 系统委托任务
%%     %% ets:new(?ETS_TASK_CONSIGN, [{keypos, #ets_task_consign.id}, named_table, public, set]),
%%     %% 角色委托任务列表
%%     %% ets:new(?ETS_CONSIGN_TASK, [{keypos, #ets_consign_task.id}, named_table, public, set]),
%%     %% 委托任务角色数值列表
%%     %% ets:new(?ETS_CONSIGN_PLAYER, [{keypos, #ets_consign_player.pid}, named_table, public, set]),
%%     %% 离线经验累积
%%     %% ets:new(?ETS_OFFLINE_AWARD,[{keypos, #ets_offline_award.pid}, named_table, public, set]),
%%     %% 在线累积奖励
%%     %% ets:new(?ETS_ONLINE_AWARD,[{keypos, #ets_online_award.pid}, named_table, public, set]),
...
%%     %% 节日登陆奖励
%%     %% ets:new(?ETS_ONLINE_AWARD_HOLIDAY,[{keypos, #ets_online_award_holiday.pid}, named_table, public, set]),
%%     %% 英雄帖表 
%%     %% ets:new(?ETS_HERO_CARD,
%%     %%         [{keypos, #ets_hero_card.pid}, named_table, public, set]),
%%     %% ets:new(?ETS_BASE_HERO_CARD, [{keypos, #ets_base_hero_card.id}, named_table, public, set]),
...
%%             [{keypos, #ets_base_parter.parter_id}, named_table, public, set]),
%%     ets:new(?ETS_BASE_PARTNER_GROW,
%%             [{keypos, #ets_base_partner_grow.lv}, named_table, public, set]),
%%     ets:new(?ETS_BASE_PARTNER_BEYOND,
%%             [{keypos, #ets_base_partner_beyond.id}, 
%%              named_table, public, set, {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_PARTNER_PRAY,
%%             [{keypos, #ets_base_partner_pray.id}, named_table, public,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_PARTNER_DESTINY,
%%             [{keypos, #rec_base_partner_destiny.partner_uid}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_PARTNER_FATE,
%%             [{keypos, #rec_base_partner_fate.fate_id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% 角色伙伴表
%%     ets:new(?ETS_PARTER, [{keypos, #ets_parter.id}, named_table, public, set]),
%%     ets:new(?ETS_PARTNER_RANK,
%%             [{keypos, #rec_partner_rank.key}, named_table, public, set]),

%%     %% 离线伙伴数据
%%     %% 离线角色装备表
%%     ets:new(?ETS_OFFLINE_PARTER,
%%             [{keypos, #ets_parter.id}, named_table, public, set]),
%%     ets:new(?ETS_OFFLINE_GOODS, [{keypos,#goods.id},named_table, public, set]),

%%     %% 装备强化规则表
%%     ets:new(?ETS_BASE_EQUIP_STRENGTHEN_RULE,
%%             [{keypos, #ets_base_equip_strengthen_rule.id}, named_table, public, set]),
%%     %% 装备套装基础表
%%     %% ets:new(?ETS_BASE_EQUIP_SUIT,
%%     %%         [{keypos,#ets_base_equip_suit.suit_id},named_table, public, set]),
%%     %% 装备套装属性表
%%     %% ets:new(?ETS_BASE_EQUIP_SUIT_ATTRIBUTE,
%%     %%         [{keypos,#ets_base_equip_suit_attribute.id},named_table, public, set]),
%%     %% 装备冶炼规则表
%%     %% ets:new(?ETS_BASE_EQUIP_PRACTISE_RULE,
%%     %%         [{keypos,#ets_base_equip_practise_rule.id},named_table, public, set]),
%%     %% 冶炼星级表
%%     ets:new(?ETS_BASE_EQUIP_PRACTISE_STAR,
%%             [{keypos, #ets_base_practise_star.id}, named_table, public, set, 
%%              {read_concurrency, true}]),
%%     %% 冶炼属性个数表
%%     ets:new(?ETS_BASE_EQUIP_PRACTISE_NUM,
%%             [{keypos, #ets_base_practise_num.id}, named_table, public, set]),
%%     %% 冶炼属性配置表
%%     ets:new(?ETS_BASE_EQUIP_PRACTISE_ATTRIBUTE,
%%             [{keypos, #ets_base_practise_attribute.id}, named_table, public, set]),
%%     %% 装备强化属性提升表
%%     ets:new(?ETS_BASE_EQUIP_ATTRIBUTE,
%%             [{keypos, #rec_base_equip_attribute.id}, named_table, public, set]),
%%     %% 怪物掉落控制数据
%%     ets:new(?ETS_BASE_GOODS_DROP_CONTROL,
%%             [{keypos, #ets_base_goods_drop_control.goods_id}, named_table, public, set]),
%%     %% 基础buff表
%%     ets:new(?ETS_BASE_BUFF,
%%             [{keypos, #ets_base_buff.buff_id}, named_table, public, set]),
%%     %% 玩家信息表
%%     ets:new(?ETS_PLAYER_INFO,
%%             [{keypos, #ets_player_info.cash_id}, named_table, public, set]),

%%     %% 基础训练师表
%%     ets:new(?ETS_BASE_TRAINER,
%%             [{keypos, #ets_base_trainer.id}, named_table, public, set]),
...
%%             [{keypos, #ets_base_arena_member.id}, named_table, public, set]),
%%     ets:new(?ETS_BASE_ARENA_MONSTER,
%%             [{keypos, #ets_mon.unique_key}, named_table, public, set]),
%%     ets:new(?ETS_BASE_ARENA_REWARD,
%%             [{keypos, #ets_base_arena_reward.id}, named_table, public, set]),
%%     ets:new(?ETS_BASE_ARENA_WINNING_REWARD,
%%             [{keypos, #ets_base_arena_winning_reward.id}, named_table, public, set]), 
...
%%             [{keypos, #ets_treasure_equip.player_id}, named_table, public, set]),
    
%%     ets:new(?ETS_TREASURE_EQUIP_OFFLINE,
%%             [{keypos, #ets_treasure_equip.player_id}, named_table, public, set]),
...
%%     %% 分线数据
%%     ets:new(?ETS_LINE_COUNT, 
%%             [{keypos, #line_count.key}, named_table, public, set]),

%%     %% ets:new(?ETS_BASE_GUILD_MAGIC, [{keypos, #ets_base_guild_magic.id}, named_table, public, set]),
%%     %% 玩家术法表
%%     %% ets:new(?ETS_PLAYER_MAGIC, [{keypos, #ets_player_magic.id}, named_table, public, set]),
%%     %% 主动技能表
%%     ets:new(?ETS_COMBAT_SKILL,
%%             [{keypos, #rec_combat_skill.id}, named_table, public, set]),
%%     %% 被技能表
%%     ets:new(?ETS_COMBAT_SKILL_PASSIVE,
%%             [{keypos,#rec_combat_skill_passive.id}, named_table, public, set]),
...
%%     ets:new(?ETS_BASE_BEAST_KEEP_ATTRI,
%%             [{keypos, #rec_base_beast_keep_attri.id}, named_table, public, set,
%%              {read_concurrency, true}]),
...
%%     ets:new(?ETS_BASE_BEAST_CAMP,
%%             [{keypos, #rec_base_beast_camp.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     %% ets:new(?ETS_BASE_BEAST_CAMP_RULE,[{keypos, #rec_base_beast_camp_rule.id}, named_table, public, set]),
%%     ets:new(?ETS_BEAST,
%%             [{keypos, #rec_beast.id}, named_table, public, set]),
%%     ets:new(?ETS_CAMP,[{keypos,#rec_camp.id}, named_table, public ,set]),

%%     ets:new(?ETS_BASE_FIGHT_SOUL,
%%             [{keypos, #rec_base_fight_soul.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_FIGHT_SOUL,
%%             [{keypos, #rec_fight_soul.id}, named_table, public, set]),

%%     %% 副本列表
%%     ets:new(?ETS_PLAYER_DUMGEON,
%%             [{keypos, #player_dungeon.id}, named_table, public, set]),

%%     %% 战报
%%     ets:new(?ETS_FIGHT_REPORT,
%%             [{keypos, #rec_fight_report.uuid}, named_table, public, set]),
%%     %% 战报查询使用
%%     ets:new(?ETS_FIGHT_REPORT_INFO,
%%             [{keypos, #rec_fight_report_info.key}, named_table, public, set]),

%%     %% 神秘商店
%%     ets:new(?ETS_MYSTERY_SHOP,
%%             [{keypos, #ets_player_mystery_shop.player_id}, named_table, public, set]),

%%     %% 星力配置表
%%     ets:new(?ETS_BASE_STAR_POWER,
%%             [{keypos,#rec_base_star_power.id}, named_table, public, set,
%%              {read_concurrency, true}]),

%%     %% 战斗属性相关
%%     ets:new(?ETS_PLAYER_COMBAT_ATTRI,
%%             [{keypos, #rec_player_combat_attri.player_id}, named_table, public ,set]),
%%     ets:new(?ETS_PLAYER_COMBAT_ATTRIBUTE_EX,
%%             [{keypos, #ets_player_combat_attribute_ex.id}, named_table, public ,set]),

%%     %% VIP相关
%%     ets:new(?ETS_BASE_VIP, [{keypos, #rec_base_vip.vip_lv}, named_table, public ,set]),

%%     %% 过关斩将相关ETS表
%%     ets:new(?ETS_BASE_KILO_RIDE,
%%             [{keypos, #rec_base_kilo_ride.layer_id}, named_table, public, set,
%%              {read_concurrency, true}]),
...
%%             [{keypos, #rec_base_kilo_ride_rank_reward.id}, named_table, public ,set]),
%%     ets:new(?ETS_KILO_RIDE,
%%             [{keypos, #rec_kilo_ride.player_id}, named_table, public, set]),

%%     ets:new(?ETS_BASE_CHEST,
%%             [{keypos, #rec_base_chest.id}, named_table, public, set,
%%              {read_concurrency, true}]),

%%     %% 试炼/小助手相关
%%     ets:new(?ETS_BASE_TRIAL,
%%             [{keypos, #base_trial_data.trial_id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_TRIAL_REWARD,
%%             [{keypos, #base_trial_award.reward_id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_PLAYER_TRIAL,
%%             [{keypos, #dic_player_trial.id}, named_table, public, set]),

%%     %% 活动相关
%%     ets:new(?ETS_BASE_ACTIVITY, 
%%             [{keypos, #base_activity.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_OP_ACTIVITIES, 
%%             [{keypos, #ets_base_op_activities.id}, 
%%              named_table, public, set, {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_ACTIVITY_RATE,
%%             [{keypos, #rec_base_activity_rate.id}, named_table, public, set]),

%%     %% 世界boss
%%     ets:new(?ETS_BASE_BOSS,
%%             [{keypos, #base_boss.scene}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_BOSS_UPGRADE,
%%             [{keypos, #base_boss_upgrade.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_WORLD_BOSS,
%%             [{keypos, #mod_boss_state.boss_scene_id}, named_table, public, set]),

%%     %% 南蛮入侵
%%     ets:new(?ETS_SOUTHER_BOSS,
%%             [{keypos, #rec_southern_boss.key}, named_table, public ,set]),
%%     ets:new(?ETS_SOUTHER_INFO,
%%             [{keypos, #rec_southern_info.player_id}, named_table, public ,set]),
%%     ets:new(?ETS_BASE_SOUTHER_UPGRADE,
%%             [{keypos, #rec_base_southern_upgrade.layer}, named_table, public ,set]),
...
%%     %% 系统公告
%%     ets:new(?ETS_BASE_SYS_ACM,
%%             [{keypos, #rec_base_sys_acm.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_SYS_ACM,
%%             [{keypos, #rec_sys_acm.id}, named_table, public, set]),
%%     ets:new(?ETS_RANK_PLAYER,
%%             [{keypos, #rec_rank_data.id}, named_table, public, ordered_set]),

%%     ets:new(?ETS_WHITE_IP, [named_table, public, set]),

%%     %% 帮派配置表
%%     ets:new(?ETS_BASE_GUILD,
%%             [{keypos,#ets_base_guild.lv}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_GUILD_SKILL,
%%             [{keypos, #ets_base_guild_skill.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_GUILD_DONATE,
%%             [{keypos, #ets_base_guild_donate.id}, named_table, public, set,
%%              {read_concurrency, true}]),
%%     ets:new(?ETS_BASE_GUILD_POSITION,
%%             [{keypos, #ets_base_guild_position.id}, named_table, public, set,
%%              {read_concurrency, true}]), 
%%     %% 帮派数据ETS表
%%     ets:new(?ETS_GUILD,
%%             [{keypos, #ets_guild.id},named_table,public,set]),
%%     ets:new(?ETS_GUILD_SKILL,
%%             [{keypos, #ets_guild_skill.id},named_table,public,set]),
%%     ets:new(?ETS_GUILD_APPLY,
%%             [{keypos,#ets_guild_apply.id},named_table,public,set]),
%%     ets:new(?ETS_GUILD_INVITE,
%%             [{keypos, #ets_guild_invite.id},named_table,public,set]),
%%     ets:new(?ETS_GUILD_MEMBER,
%%             [{keypos, #ets_guild_member.player_id},named_table,public,set]),
%%     ets:new(?ETS_GUILD_LOG,
%%             [{keypos, #ets_guild_log.id},named_table,public,set]),
%%     ets:new(?ETS_GUILD_MAGIC,
%%             [{keypos, #ets_guild_magic.id}, named_table, public, set]),
%%     %% 军团战斗积分排名表
%%     ets:new(?ETS_GUILD_SCORE,
%%             [{keypos, #rec_guild_score.guild_id}, named_table, public, set]),
%%     %% 军团战列表
%%     ets:new(?ETS_GUILD_FIGHT_LIST,
%%             [{keypos, #rec_guild_fight_list.key}, named_table, public, set]),
%%     %% 军团战八强列表，供非积分时候查询
%%     ets:new(?ETS_GUILD_FIGHT_LIST_DUMP,
%%             [{keypos, #rec_guild_fight_list.key}, named_table, public, set]),

%%     %% 军团战列表
%%     ets:new(?ETS_GUILD_FIGHT_MEMBER,
%%             [{keypos, #rec_guild_fight_member.key}, named_table, public, set]),
%%     %% 军团战战报列表
%%     ets:new(?ETS_GUILD_FIGHT_REPORT,
%%             [{keypos, #rec_guild_fight_report.key}, named_table, public, set]),
%%     %% 军团战世界8强玩家
%%     ets:new(?ETS_GUILD_FIGHT_MEMBER_BOARD,
%%             [{keypos, #rec_guild_fight_member_board.player_id}, named_table, public, set]),
%%     %% 军团战世界8强军团
%%     ets:new(?ETS_GUILD_FIGHT_GUILD_BOARD,
%%             [{keypos, #rec_guild_fight_guild_board.guild_id}, named_table, public, set]),
%%     %% 军团战投注表
%%     ets:new(?ETS_GUILD_FIGHT_GAMBLE,
%%             [{keypos, #rec_guild_fight_gamble.id}, named_table, public, set]),
%%     %% 8强排名奖励表
%%     ets:new(?ETS_BASE_EIGHT_FIGHT_RANK_REWARD,
%%             [{keypos, #rec_base_eight_fight_rank_reward.id}, named_table, public, set]),

%%     %% 等级标准表 
%%     ets:new(?ETS_BASE_LV,
%%             [{keypos, #rec_base_lv.lv}, named_table, public, set]),

%%     %% 成就系统
%%     %% 成就基础数据
%%     ets:new(?ETS_BASE_ACHIEVE,
%%             [{keypos, #base_achieve.achieve_id}, named_table, public, set]),
%%     %% 玩家成就数据
%%     ets:new(?ETS_PLAYER_ACHIEVE,
%%             [{keypos, #dic_player_achieve.id}, named_table, public, set]),
%%     %% 玩家某类次数，暂不做
%%     %% ets:new(?ETS_PLAYER_ACHIEVE_TIMES,
%%     %%         [{keypos, #dic_player_achieve_times.id}, named_table, public, set]),
...
%%     %% 新手礼包激活码配置表
%%     ets:new(?ETS_ACTIVATION_CODE,
%%             [{keypos, #rec_activation_code.ac_code}, named_table, public, set]),

%%     ets:new(?ETS_UPS_FIGHTER,
%%             [{keypos, #rec_ups_fighter.key}, named_table, public, set]),
%%     ets:new(?ETS_UPS_FIGHT_REPORT,
%%             [{keypos, #rec_ups_fight_report.id}, named_table, public, set]),

%%     %% 给苹果服务器发送的receipt的储存
%%     ets:new(?ETS_STORE_RECEIPT,
%%             [{keypos, #rec_store_receipt.id}, named_table, public, set]),
%%     %% 苹果平台product
%%     ets:new(?ETS_APP_STORE_PRODUCT,
%%             [{keypos, #rec_app_store_product.product_id}, named_table, public, set]),
%%     ok.
