...
-export([encode_pbskill/1, decode_pbskill/1,
	 ...
	 encode_pbfriendlist/1, decode_pbfriendlist/1,
	 delimited_decode_pbfriendlist/1, encode_pbfriend/1,
...
-record(pbfriendlist, {update_list, delete_list}).
...
encode_pbfriendlist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbfriendlist(Record)
    when is_record(Record, pbfriendlist) ->
    encode(pbfriendlist, Record).
...
encode(pbfriendlist, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbfriendlist, Record) ->
    [iolist(pbfriendlist, Record)
     | encode_extensions(Record)];
...
iolist(pbfriendlist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbfriendlist.update_list, none),
	  pbfriend, []),
     pack(2, repeated,
	  with_default(Record#pbfriendlist.delete_list, none),
	  pbfriend, [])];
...
decode_pbfriendlist(Bytes) when is_binary(Bytes) ->
    decode(pbfriendlist, Bytes).
...
delimited_decode_pbfriendlist(Bytes) ->
    delimited_decode(pbfriendlist, Bytes).
...
decode(pbfriendlist, Bytes) when is_binary(Bytes) ->
    Types = [{2, delete_list, pbfriend,
	      [is_record, repeated]},
	     {1, update_list, pbfriend, [is_record, repeated]}],
    Defaults = [{1, update_list, []}, {2, delete_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbfriendlist, Decoded);
...
to_record(pbfriend, DecodedTuples) ->
    ...
    Record1;
to_record(pbfriendlist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbfriendlist),
