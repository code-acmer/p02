...
-export([encode_pbskill/1, decode_pbskill/1,
	 ...
	 delimited_decode_pbfriend/1, encode_pbfashionlist/1,
	 decode_pbfashionlist/1,
	 delimited_decode_pbfashionlist/1, encode_pbfashion/1,
	 decode_pbfashion/1, delimited_decode_pbfashion/1,
...
-record(pbfashionlist,
	{current_id, change_id, fashion_list}).

-record(pbfashion, {id, nid, base_id, container}).
...
encode_pbfashionlist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbfashionlist(Record)
    when is_record(Record, pbfashionlist) ->
    encode(pbfashionlist, Record).

encode_pbfashion(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbfashion(Record)
    when is_record(Record, pbfashion) ->
    encode(pbfashion, Record).
...
encode(pbcombatreward, Record) ->
    [iolist(pbcombatreward, Record)
     | encode_extensions(Record)];
encode(pbfashion, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbfashion, Record) ->
    [iolist(pbfashion, Record) | encode_extensions(Record)];
encode(pbfashionlist, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbfashionlist, Record) ->
    [iolist(pbfashionlist, Record)
     | encode_extensions(Record)];
...
iolist(pbcombatreward, Record) ->
    [pack(1, optional,
     ...
     pack(8, repeated,
	  ...
	  int64, [])];
iolist(pbfashion, Record) ->
    [pack(1, optional,
	  with_default(Record#pbfashion.id, none), int32, []),
     pack(2, optional,
	  with_default(Record#pbfashion.nid, none), int32, []),
     pack(3, optional,
	  with_default(Record#pbfashion.base_id, none), int32,
	  []),
     pack(6, optional,
	  with_default(Record#pbfashion.container, none), int32,
	  [])];
iolist(pbfashionlist, Record) ->
    [pack(1, optional,
	  with_default(Record#pbfashionlist.current_id, none),
	  int32, []),
     pack(2, optional,
	  with_default(Record#pbfashionlist.change_id, none),
	  int32, []),
     pack(3, repeated,
	  with_default(Record#pbfashionlist.fashion_list, none),
	  pbfashion, [])];
...
decode_pbfashionlist(Bytes) when is_binary(Bytes) ->
    decode(pbfashionlist, Bytes).

decode_pbfashion(Bytes) when is_binary(Bytes) ->
    decode(pbfashion, Bytes).
...
delimited_decode_pbfashion(Bytes) ->
    delimited_decode(pbfashion, Bytes).

delimited_decode_pbfashionlist(Bytes) ->
    delimited_decode(pbfashionlist, Bytes).
...
decode(pbfashion, Bytes) when is_binary(Bytes) ->
    Types = [{6, container, int32, []},
	     {3, base_id, int32, []}, {2, nid, int32, []},
	     {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbfashion, Decoded);
decode(pbfashionlist, Bytes) when is_binary(Bytes) ->
    Types = [{3, fashion_list, pbfashion,
	      [is_record, repeated]},
	     {2, change_id, int32, []}, {1, current_id, int32, []}],
    Defaults = [{3, fashion_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbfashionlist, Decoded);
...
to_record(pbcombatreward, DecodedTuples) ->
    ...
    Record1;
to_record(pbfashion, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbfashion),
			  ...
			  #pbfashion{}, DecodedTuples),
    Record1;
to_record(pbfashionlist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbfashionlist),
