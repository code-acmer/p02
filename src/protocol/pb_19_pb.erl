...
-export([encode_pbskill/1, decode_pbskill/1,
	 ...
	 delimited_decode_pbresult/1, encode_pbmaillist/1,
	 decode_pbmaillist/1, delimited_decode_pbmaillist/1,
	 encode_pbmailgoods/1, decode_pbmailgoods/1,
	 delimited_decode_pbmailgoods/1, encode_pbmail/1,
	 decode_pbmail/1, delimited_decode_pbmail/1,
...
-record(pbmaillist, {update_list, delete_list}).

-record(pbmailgoods, {goodsid, goodsum, position}).

-record(pbmail,
	{id, type, timestamp, sender_name, sender_id, recv_id,
	 recv_name, title, content, goodslist, state,
	 battle_report}).
...
encode_pbmaillist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbmaillist(Record)
    when is_record(Record, pbmaillist) ->
    encode(pbmaillist, Record).

encode_pbmailgoods(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbmailgoods(Record)
    when is_record(Record, pbmailgoods) ->
    encode(pbmailgoods, Record).

encode_pbmail(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbmail(Record) when is_record(Record, pbmail) ->
    encode(pbmail, Record).
...
encode(pbidstring, Record) ->
    [iolist(pbidstring, Record)
     | encode_extensions(Record)];
encode(pbmail, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbmail, Record) ->
    [iolist(pbmail, Record) | encode_extensions(Record)];
encode(pbmailgoods, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbmailgoods, Record) ->
    [iolist(pbmailgoods, Record)
     | encode_extensions(Record)];
encode(pbmaillist, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbmaillist, Record) ->
    [iolist(pbmaillist, Record)
     | encode_extensions(Record)];
...
iolist(pbmail, Record) ->
    [pack(1, optional, with_default(Record#pbmail.id, none),
	  int32, []),
     pack(2, optional,
	  with_default(Record#pbmail.type, none), int32, []),
     pack(3, optional,
	  with_default(Record#pbmail.timestamp, none), int32, []),
     pack(4, optional,
	  with_default(Record#pbmail.sender_name, none), string,
	  []),
     pack(5, optional,
	  with_default(Record#pbmail.sender_id, none), int64, []),
     pack(6, optional,
	  with_default(Record#pbmail.recv_id, none), int64, []),
     pack(7, optional,
	  with_default(Record#pbmail.recv_name, none), string,
	  []),
     pack(8, optional,
	  with_default(Record#pbmail.title, none), string, []),
     pack(9, optional,
	  with_default(Record#pbmail.content, none), string, []),
     pack(10, repeated,
	  with_default(Record#pbmail.goodslist, none),
	  pbmailgoods, []),
     pack(11, optional,
	  with_default(Record#pbmail.state, none), int32, []),
     pack(12, optional,
	  with_default(Record#pbmail.battle_report, none), string,
	  [])];
iolist(pbmailgoods, Record) ->
    [pack(1, optional,
	  with_default(Record#pbmailgoods.goodsid, none), int64,
	  []),
     pack(2, optional,
	  with_default(Record#pbmailgoods.goodsum, none), int32,
	  []),
     pack(3, optional,
	  with_default(Record#pbmailgoods.position, none), int32,
	  [])];
iolist(pbmaillist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbmaillist.update_list, none),
	  pbmail, []),
     pack(2, repeated,
	  with_default(Record#pbmaillist.delete_list, none),
	  pbmail, [])];
...
decode_pbmaillist(Bytes) when is_binary(Bytes) ->
    decode(pbmaillist, Bytes).

decode_pbmailgoods(Bytes) when is_binary(Bytes) ->
    decode(pbmailgoods, Bytes).

decode_pbmail(Bytes) when is_binary(Bytes) ->
    decode(pbmail, Bytes).
...
delimited_decode_pbmail(Bytes) ->
    delimited_decode(pbmail, Bytes).

delimited_decode_pbmailgoods(Bytes) ->
    delimited_decode(pbmailgoods, Bytes).

delimited_decode_pbmaillist(Bytes) ->
    delimited_decode(pbmaillist, Bytes).
...
decode(pbmail, Bytes) when is_binary(Bytes) ->
    Types = [{12, battle_report, string, []},
	     {11, state, int32, []},
	     {10, goodslist, pbmailgoods, [is_record, repeated]},
	     {9, content, string, []}, {8, title, string, []},
	     {7, recv_name, string, []}, {6, recv_id, int64, []},
	     {5, sender_id, int64, []}, {4, sender_name, string, []},
	     {3, timestamp, int32, []}, {2, type, int32, []},
	     {1, id, int32, []}],
    Defaults = [{10, goodslist, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbmail, Decoded);
decode(pbmailgoods, Bytes) when is_binary(Bytes) ->
    Types = [{3, position, int32, []},
	     {2, goodsum, int32, []}, {1, goodsid, int64, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbmailgoods, Decoded);
decode(pbmaillist, Bytes) when is_binary(Bytes) ->
    Types = [{2, delete_list, pbmail,
	      [is_record, repeated]},
	     {1, update_list, pbmail, [is_record, repeated]}],
    Defaults = [{1, update_list, []}, {2, delete_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbmaillist, Decoded);
...
to_record(pbidstring, DecodedTuples) ->
    ...
    Record1;
to_record(pbmail, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, pbmail),
						   Record, Name, Val)
			  end,
			  #pbmail{}, DecodedTuples),
    Record1;
to_record(pbmailgoods, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbmailgoods),
			  ...
			  #pbmailgoods{}, DecodedTuples),
    Record1;
to_record(pbmaillist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbmaillist),
