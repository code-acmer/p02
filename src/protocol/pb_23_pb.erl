...
-export([encode_pbskill/1, decode_pbskill/1,
	 ...
	 delimited_decode_pbfriend/1, encode_pbcrossrecord/1,
	 decode_pbcrossrecord/1,
	 delimited_decode_pbcrossrecord/1,
	 encode_pbcrossisland/1, decode_pbcrossisland/1,
	 delimited_decode_pbcrossisland/1,
	 encode_pbcrosshistorylist/1,
	 decode_pbcrosshistorylist/1,
	 delimited_decode_pbcrosshistorylist/1,
	 encode_pbcrosshistory/1, decode_pbcrosshistory/1,
	 delimited_decode_pbcrosshistory/1,
	 encode_pbcrossfighter/1, decode_pbcrossfighter/1,
	 delimited_decode_pbcrossfighter/1,
	 ...
	 encode_pbarenainfolist/1, decode_pbarenainfolist/1,
	 delimited_decode_pbarenainfolist/1,
	 encode_pbarenainfo/1, decode_pbarenainfo/1,
	 delimited_decode_pbarenainfo/1,
	 encode_pbarenachallenge/1, decode_pbarenachallenge/1,
	 delimited_decode_pbarenachallenge/1,
	 encode_pbarenabattlereportlist/1,
	 decode_pbarenabattlereportlist/1,
	 delimited_decode_pbarenabattlereportlist/1,
	 encode_pbarenabattlereport/1,
	 decode_pbarenabattlereport/1,
	 delimited_decode_pbarenabattlereport/1]).
...
-record(pbcrossrecord,
	{timestamp, atk_id, atk_name, def_id, def_name,
	 result}).

-record(pbcrossisland,
	{id, occupy_id, occupy_name, occupy_career, occupy_lv,
	 occupy_sn, occupy_ability, occupy_robot, occupy_time,
	 calc_timestamp, is_enemy}).

-record(pbcrosshistorylist, {list}).

-record(pbcrosshistory, {rank, round, timestamp}).

-record(pbcrossfighter,
	{times, buy_times, islands, records}).
...
-record(pbarenainfolist, {arena_player_list}).

-record(pbarenainfo,
	{challenge_times, rest_challenge_times,
	 challenge_timestamp, rank, win_times, fail_times, robot,
	 player_id, level, nickname, career, battle_ability,
	 enemy, sn, buy_times, score, robot_id}).

-record(pbarenachallenge, {id, result, robot}).

-record(pbarenabattlereportlist, {report_list}).

-record(pbarenabattlereport,
	{id, type, result, timestamp, player_id, nickname,
	 deffender_id, deffender_name, rank_change_type,
	 attack_rank, deffender_rank}).
...
encode_pbcrossrecord(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbcrossrecord(Record)
    when is_record(Record, pbcrossrecord) ->
    encode(pbcrossrecord, Record).

encode_pbcrossisland(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbcrossisland(Record)
    when is_record(Record, pbcrossisland) ->
    encode(pbcrossisland, Record).

encode_pbcrosshistorylist(Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode_pbcrosshistorylist(Record)
    when is_record(Record, pbcrosshistorylist) ->
    encode(pbcrosshistorylist, Record).

encode_pbcrosshistory(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbcrosshistory(Record)
    when is_record(Record, pbcrosshistory) ->
    encode(pbcrosshistory, Record).

encode_pbcrossfighter(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbcrossfighter(Record)
    when is_record(Record, pbcrossfighter) ->
    encode(pbcrossfighter, Record).
...
encode_pbarenainfolist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbarenainfolist(Record)
    when is_record(Record, pbarenainfolist) ->
    encode(pbarenainfolist, Record).

encode_pbarenainfo(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbarenainfo(Record)
    when is_record(Record, pbarenainfo) ->
    encode(pbarenainfo, Record).

encode_pbarenachallenge(Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode_pbarenachallenge(Record)
    when is_record(Record, pbarenachallenge) ->
    encode(pbarenachallenge, Record).

encode_pbarenabattlereportlist(Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode_pbarenabattlereportlist(Record)
    when is_record(Record, pbarenabattlereportlist) ->
    encode(pbarenabattlereportlist, Record).

encode_pbarenabattlereport(Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode_pbarenabattlereport(Record)
    when is_record(Record, pbarenabattlereport) ->
    encode(pbarenabattlereport, Record).

encode(pbarenabattlereport, Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode(pbarenabattlereport, Record) ->
    [iolist(pbarenabattlereport, Record)
     | encode_extensions(Record)];
encode(pbarenabattlereportlist, Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode(pbarenabattlereportlist, Record) ->
    [iolist(pbarenabattlereportlist, Record)
     | encode_extensions(Record)];
encode(pbarenachallenge, Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode(pbarenachallenge, Record) ->
    [iolist(pbarenachallenge, Record)
     | encode_extensions(Record)];
encode(pbarenainfo, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbarenainfo, Record) ->
    [iolist(pbarenainfo, Record)
     | encode_extensions(Record)];
encode(pbarenainfolist, Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode(pbarenainfolist, Record) ->
    [iolist(pbarenainfolist, Record)
     | encode_extensions(Record)];
...
encode(pbcombatreward, Record) ->
    [iolist(pbcombatreward, Record)
     | encode_extensions(Record)];
encode(pbcrossfighter, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbcrossfighter, Record) ->
    [iolist(pbcrossfighter, Record)
     | encode_extensions(Record)];
encode(pbcrosshistory, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbcrosshistory, Record) ->
    [iolist(pbcrosshistory, Record)
     | encode_extensions(Record)];
encode(pbcrosshistorylist, Records)
    when is_list(Records) ->
    delimited_encode(Records);
encode(pbcrosshistorylist, Record) ->
    [iolist(pbcrosshistorylist, Record)
     | encode_extensions(Record)];
encode(pbcrossisland, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbcrossisland, Record) ->
    [iolist(pbcrossisland, Record)
     | encode_extensions(Record)];
encode(pbcrossrecord, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbcrossrecord, Record) ->
    [iolist(pbcrossrecord, Record)
     | encode_extensions(Record)];
...
iolist(pbarenabattlereport, Record) ->
    [pack(1, optional,
	  with_default(Record#pbarenabattlereport.id, none),
	  int32, []),
     pack(2, optional,
	  with_default(Record#pbarenabattlereport.type, none),
	  int32, []),
     pack(3, optional,
	  with_default(Record#pbarenabattlereport.result, none),
	  int32, []),
     pack(4, optional,
	  with_default(Record#pbarenabattlereport.timestamp,
		       none),
	  int32, []),
     pack(5, optional,
	  with_default(Record#pbarenabattlereport.player_id,
		       none),
	  int32, []),
     pack(6, optional,
	  with_default(Record#pbarenabattlereport.nickname, none),
	  string, []),
     pack(7, optional,
	  with_default(Record#pbarenabattlereport.deffender_id,
		       none),
	  int32, []),
     pack(8, optional,
	  with_default(Record#pbarenabattlereport.deffender_name,
		       none),
	  string, []),
     pack(9, optional,
	  with_default(Record#pbarenabattlereport.rank_change_type,
		       none),
	  int32, []),
     pack(10, optional,
	  with_default(Record#pbarenabattlereport.attack_rank,
		       none),
	  int32, []),
     pack(11, optional,
	  with_default(Record#pbarenabattlereport.deffender_rank,
		       none),
	  int32, [])];
iolist(pbarenabattlereportlist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbarenabattlereportlist.report_list,
		       none),
	  pbarenabattlereport, [])];
iolist(pbarenachallenge, Record) ->
    [pack(1, optional,
	  with_default(Record#pbarenachallenge.id, none), int64,
	  []),
     pack(2, optional,
	  with_default(Record#pbarenachallenge.result, none),
	  int32, []),
     pack(3, optional,
	  with_default(Record#pbarenachallenge.robot, none),
	  int32, [])];
iolist(pbarenainfo, Record) ->
    [pack(1, optional,
	  with_default(Record#pbarenainfo.challenge_times, none),
	  int32, []),
     pack(2, optional,
	  with_default(Record#pbarenainfo.rest_challenge_times,
		       none),
	  int32, []),
     pack(3, optional,
	  with_default(Record#pbarenainfo.challenge_timestamp,
		       none),
	  int32, []),
     pack(4, optional,
	  with_default(Record#pbarenainfo.rank, none), int32, []),
     pack(5, optional,
	  with_default(Record#pbarenainfo.win_times, none), int32,
	  []),
     pack(6, optional,
	  with_default(Record#pbarenainfo.fail_times, none),
	  int32, []),
     pack(7, optional,
	  with_default(Record#pbarenainfo.robot, none), int32,
	  []),
     pack(8, optional,
	  with_default(Record#pbarenainfo.player_id, none), int64,
	  []),
     pack(9, optional,
	  with_default(Record#pbarenainfo.level, none), int32,
	  []),
     pack(10, optional,
	  with_default(Record#pbarenainfo.nickname, none), string,
	  []),
     pack(11, optional,
	  with_default(Record#pbarenainfo.career, none), int32,
	  []),
     pack(12, optional,
	  with_default(Record#pbarenainfo.battle_ability, none),
	  int32, []),
     pack(13, repeated,
	  with_default(Record#pbarenainfo.enemy, none), int64,
	  []),
     pack(14, optional,
	  with_default(Record#pbarenainfo.sn, none), int32, []),
     pack(15, optional,
	  with_default(Record#pbarenainfo.buy_times, none), int32,
	  []),
     pack(16, optional,
	  with_default(Record#pbarenainfo.score, none), int32,
	  []),
     pack(17, optional,
	  with_default(Record#pbarenainfo.robot_id, none), int32,
	  [])];
iolist(pbarenainfolist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbarenainfolist.arena_player_list,
		       none),
	  pbarenainfo, [])];
iolist(pbattribute, Record) ->
    [pack(1, optional,
...
iolist(pbcrossfighter, Record) ->
    [pack(1, optional,
	  with_default(Record#pbcrossfighter.times, none), int32,
	  []),
     pack(2, optional,
	  with_default(Record#pbcrossfighter.buy_times, none),
	  int32, []),
     pack(3, repeated,
	  with_default(Record#pbcrossfighter.islands, none),
	  pbcrossisland, []),
     pack(4, repeated,
	  with_default(Record#pbcrossfighter.records, none),
	  pbcrossrecord, [])];
iolist(pbcrosshistory, Record) ->
    [pack(1, optional,
	  with_default(Record#pbcrosshistory.rank, none), int32,
	  []),
     pack(2, optional,
	  with_default(Record#pbcrosshistory.round, none), int32,
	  []),
     pack(3, optional,
	  with_default(Record#pbcrosshistory.timestamp, none),
	  int32, [])];
iolist(pbcrosshistorylist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbcrosshistorylist.list, none),
	  pbcrosshistory, [])];
iolist(pbcrossisland, Record) ->
    [pack(1, optional,
	  with_default(Record#pbcrossisland.id, none), int32, []),
     pack(2, optional,
	  with_default(Record#pbcrossisland.occupy_id, none),
	  int64, []),
     pack(3, optional,
	  with_default(Record#pbcrossisland.occupy_name, none),
	  string, []),
     pack(4, optional,
	  with_default(Record#pbcrossisland.occupy_career, none),
	  int32, []),
     pack(5, optional,
	  with_default(Record#pbcrossisland.occupy_lv, none),
	  int32, []),
     pack(6, optional,
	  with_default(Record#pbcrossisland.occupy_sn, none),
	  int32, []),
     pack(7, optional,
	  with_default(Record#pbcrossisland.occupy_ability, none),
	  int32, []),
     pack(8, optional,
	  with_default(Record#pbcrossisland.occupy_robot, none),
	  int32, []),
     pack(9, optional,
	  with_default(Record#pbcrossisland.occupy_time, none),
	  int32, []),
     pack(10, optional,
	  with_default(Record#pbcrossisland.calc_timestamp, none),
	  int32, []),
     pack(11, optional,
	  with_default(Record#pbcrossisland.is_enemy, none),
	  int32, [])];
iolist(pbcrossrecord, Record) ->
    [pack(1, optional,
	  with_default(Record#pbcrossrecord.timestamp, none),
	  int32, []),
     pack(2, optional,
	  with_default(Record#pbcrossrecord.atk_id, none), int64,
	  []),
     pack(3, optional,
	  with_default(Record#pbcrossrecord.atk_name, none),
	  string, []),
     pack(4, optional,
	  with_default(Record#pbcrossrecord.def_id, none), int64,
	  []),
     pack(5, optional,
	  with_default(Record#pbcrossrecord.def_name, none),
	  string, []),
     pack(6, optional,
	  with_default(Record#pbcrossrecord.result, none), int32,
	  [])];
...
decode_pbcrossrecord(Bytes) when is_binary(Bytes) ->
    decode(pbcrossrecord, Bytes).

decode_pbcrossisland(Bytes) when is_binary(Bytes) ->
    decode(pbcrossisland, Bytes).

decode_pbcrosshistorylist(Bytes)
    when is_binary(Bytes) ->
    decode(pbcrosshistorylist, Bytes).

decode_pbcrosshistory(Bytes) when is_binary(Bytes) ->
    decode(pbcrosshistory, Bytes).

decode_pbcrossfighter(Bytes) when is_binary(Bytes) ->
    decode(pbcrossfighter, Bytes).
...
decode_pbarenainfolist(Bytes) when is_binary(Bytes) ->
    decode(pbarenainfolist, Bytes).

decode_pbarenainfo(Bytes) when is_binary(Bytes) ->
    decode(pbarenainfo, Bytes).

decode_pbarenachallenge(Bytes) when is_binary(Bytes) ->
    decode(pbarenachallenge, Bytes).

decode_pbarenabattlereportlist(Bytes)
    when is_binary(Bytes) ->
    decode(pbarenabattlereportlist, Bytes).

decode_pbarenabattlereport(Bytes)
    when is_binary(Bytes) ->
    decode(pbarenabattlereport, Bytes).

delimited_decode_pbarenabattlereport(Bytes) ->
    delimited_decode(pbarenabattlereport, Bytes).

delimited_decode_pbarenabattlereportlist(Bytes) ->
    delimited_decode(pbarenabattlereportlist, Bytes).

delimited_decode_pbarenachallenge(Bytes) ->
    delimited_decode(pbarenachallenge, Bytes).

delimited_decode_pbarenainfo(Bytes) ->
    delimited_decode(pbarenainfo, Bytes).

delimited_decode_pbarenainfolist(Bytes) ->
    delimited_decode(pbarenainfolist, Bytes).
...
delimited_decode_pbcrossfighter(Bytes) ->
    delimited_decode(pbcrossfighter, Bytes).

delimited_decode_pbcrosshistory(Bytes) ->
    delimited_decode(pbcrosshistory, Bytes).

delimited_decode_pbcrosshistorylist(Bytes) ->
    delimited_decode(pbcrosshistorylist, Bytes).

delimited_decode_pbcrossisland(Bytes) ->
    delimited_decode(pbcrossisland, Bytes).

delimited_decode_pbcrossrecord(Bytes) ->
    delimited_decode(pbcrossrecord, Bytes).
...
decode(pbarenabattlereport, Bytes)
    when is_binary(Bytes) ->
    Types = [{11, deffender_rank, int32, []},
	     {10, attack_rank, int32, []},
	     {9, rank_change_type, int32, []},
	     {8, deffender_name, string, []},
	     {7, deffender_id, int32, []}, {6, nickname, string, []},
	     {5, player_id, int32, []}, {4, timestamp, int32, []},
	     {3, result, int32, []}, {2, type, int32, []},
	     {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbarenabattlereport, Decoded);
decode(pbarenabattlereportlist, Bytes)
    when is_binary(Bytes) ->
    Types = [{1, report_list, pbarenabattlereport,
	      [is_record, repeated]}],
    Defaults = [{1, report_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbarenabattlereportlist, Decoded);
decode(pbarenachallenge, Bytes) when is_binary(Bytes) ->
    Types = [{3, robot, int32, []}, {2, result, int32, []},
	     {1, id, int64, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbarenachallenge, Decoded);
decode(pbarenainfo, Bytes) when is_binary(Bytes) ->
    Types = [{17, robot_id, int32, []},
	     {16, score, int32, []}, {15, buy_times, int32, []},
	     {14, sn, int32, []}, {13, enemy, int64, [repeated]},
	     {12, battle_ability, int32, []},
	     {11, career, int32, []}, {10, nickname, string, []},
	     {9, level, int32, []}, {8, player_id, int64, []},
	     {7, robot, int32, []}, {6, fail_times, int32, []},
	     {5, win_times, int32, []}, {4, rank, int32, []},
	     {3, challenge_timestamp, int32, []},
	     {2, rest_challenge_times, int32, []},
	     {1, challenge_times, int32, []}],
    Defaults = [{13, enemy, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbarenainfo, Decoded);
decode(pbarenainfolist, Bytes) when is_binary(Bytes) ->
    Types = [{1, arena_player_list, pbarenainfo,
	      [is_record, repeated]}],
    Defaults = [{1, arena_player_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbarenainfolist, Decoded);
...
decode(pbcrossfighter, Bytes) when is_binary(Bytes) ->
    Types = [{4, records, pbcrossrecord,
	      [is_record, repeated]},
	     {3, islands, pbcrossisland, [is_record, repeated]},
	     {2, buy_times, int32, []}, {1, times, int32, []}],
    Defaults = [{3, islands, []}, {4, records, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcrossfighter, Decoded);
decode(pbcrosshistory, Bytes) when is_binary(Bytes) ->
    Types = [{3, timestamp, int32, []},
	     {2, round, int32, []}, {1, rank, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcrosshistory, Decoded);
decode(pbcrosshistorylist, Bytes)
    when is_binary(Bytes) ->
    Types = [{1, list, pbcrosshistory,
	      [is_record, repeated]}],
    Defaults = [{1, list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcrosshistorylist, Decoded);
decode(pbcrossisland, Bytes) when is_binary(Bytes) ->
    Types = [{11, is_enemy, int32, []},
	     {10, calc_timestamp, int32, []},
	     {9, occupy_time, int32, []},
	     {8, occupy_robot, int32, []},
	     {7, occupy_ability, int32, []},
	     {6, occupy_sn, int32, []}, {5, occupy_lv, int32, []},
	     {4, occupy_career, int32, []},
	     {3, occupy_name, string, []}, {2, occupy_id, int64, []},
	     {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcrossisland, Decoded);
decode(pbcrossrecord, Bytes) when is_binary(Bytes) ->
    Types = [{6, result, int32, []},
	     {5, def_name, string, []}, {4, def_id, int64, []},
	     {3, atk_name, string, []}, {2, atk_id, int64, []},
	     {1, timestamp, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcrossrecord, Decoded);
...
to_record(pbarenabattlereport, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			  ...
			  #pbarenabattlereport{}, DecodedTuples),
    Record1;
to_record(pbarenabattlereportlist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
			  ...
			  #pbarenabattlereportlist{}, DecodedTuples),
    Record1;
to_record(pbarenachallenge, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbarenachallenge),
			  ...
			  #pbarenachallenge{}, DecodedTuples),
    Record1;
to_record(pbarenainfo, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbarenainfo),
			  ...
			  #pbarenainfo{}, DecodedTuples),
    Record1;
to_record(pbarenainfolist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbarenainfolist),
...
to_record(pbcombatreward, DecodedTuples) ->
    ...
    Record1;
to_record(pbcrossfighter, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbcrossfighter),
			  ...
			  #pbcrossfighter{}, DecodedTuples),
    Record1;
to_record(pbcrosshistory, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbcrosshistory),
			  ...
			  #pbcrosshistory{}, DecodedTuples),
    Record1;
to_record(pbcrosshistorylist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbcrosshistorylist),
			  ...
			  #pbcrosshistorylist{}, DecodedTuples),
    Record1;
to_record(pbcrossisland, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbcrossisland),
			  ...
			  #pbcrossisland{}, DecodedTuples),
    Record1;
to_record(pbcrossrecord, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbcrossrecord),
