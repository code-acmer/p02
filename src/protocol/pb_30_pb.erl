-file("src/pb_30_pb.erl", 1).

-module(pb_30_pb).

-export([encode_pbtasklist/1, decode_pbtasklist/1,
	 delimited_decode_pbtasklist/1, encode_pbtask/1,
	 decode_pbtask/1, delimited_decode_pbtask/1,
...
-record(pbtasklist, {update, delete}).

-record(pbtask,
	{id, task_id, player_id, type, schedule, state,
	 last_op}).
...
encode_pbtasklist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbtasklist(Record)
    when is_record(Record, pbtasklist) ->
    encode(pbtasklist, Record).

encode_pbtask(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbtask(Record) when is_record(Record, pbtask) ->
    encode(pbtask, Record).
...
encode(pbtask, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbtask, Record) ->
    [iolist(pbtask, Record) | encode_extensions(Record)];
encode(pbtasklist, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbtasklist, Record) ->
    [iolist(pbtasklist, Record)
     | encode_extensions(Record)].
...
iolist(pbtask, Record) ->
    [pack(1, optional, with_default(Record#pbtask.id, none),
	  int32, []),
     pack(2, optional,
	  with_default(Record#pbtask.task_id, none), int32, []),
     pack(3, optional,
	  with_default(Record#pbtask.player_id, none), int64, []),
     pack(4, optional,
	  with_default(Record#pbtask.type, none), int32, []),
     pack(5, optional,
	  with_default(Record#pbtask.schedule, none), int32, []),
     pack(6, optional,
	  with_default(Record#pbtask.state, none), int32, []),
     pack(7, optional,
	  with_default(Record#pbtask.last_op, none), int32, [])];
iolist(pbtasklist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbtasklist.update, none), pbtask,
	  []),
     pack(2, repeated,
	  with_default(Record#pbtasklist.delete, none), pbtask,
	  [])].
...
decode_pbtasklist(Bytes) when is_binary(Bytes) ->
    decode(pbtasklist, Bytes).

decode_pbtask(Bytes) when is_binary(Bytes) ->
    decode(pbtask, Bytes).
...
delimited_decode_pbtask(Bytes) ->
    delimited_decode(pbtask, Bytes).

delimited_decode_pbtasklist(Bytes) ->
    delimited_decode(pbtasklist, Bytes).
...
decode(pbtask, Bytes) when is_binary(Bytes) ->
    Types = [{7, last_op, int32, []}, {6, state, int32, []},
	     {5, schedule, int32, []}, {4, type, int32, []},
	     {3, player_id, int64, []}, {2, task_id, int32, []},
	     {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbtask, Decoded);
decode(pbtasklist, Bytes) when is_binary(Bytes) ->
    Types = [{2, delete, pbtask, [is_record, repeated]},
	     {1, update, pbtask, [is_record, repeated]}],
    Defaults = [{1, update, []}, {2, delete, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbtasklist, Decoded).
...
to_record(pbskill, DecodedTuples) ->
    ...
    Record1;
to_record(pbtask, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, pbtask),
						   Record, Name, Val)
			  end,
			  #pbtask{}, DecodedTuples),
    Record1;
to_record(pbtasklist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbtasklist),
