-file("src/pb_34_pb.erl", 1).

-module(pb_34_pb).

-export([encode_pbtwistegglist/1,
	 decode_pbtwistegglist/1,
	 delimited_decode_pbtwistegglist/1, encode_pbtwistegg/1,
	 decode_pbtwistegg/1, delimited_decode_pbtwistegg/1,
	 ...
	 delimited_decode_pbfriend/1, encode_pbcombatreward/1,
	 decode_pbcombatreward/1,
	 delimited_decode_pbcombatreward/1, encode_pbattribute/1,
	 decode_pbattribute/1, delimited_decode_pbattribute/1,
	 encode_pbactivitylist/1, decode_pbactivitylist/1,
	 delimited_decode_pbactivitylist/1, encode_pbactivity/1,
	 decode_pbactivity/1, delimited_decode_pbactivity/1]).
...
-record(pbtwistegglist, {twist_egg}).

-record(pbtwistegg,
	{type, count, add_att_flag, goods_id}).
...
-record(pbactivitylist, {activity, mails_count}).

-record(pbactivity,
	{id, activity_desc, category, start_time, end_time,
	 server_now, type, notice, notice_start_time,
	 notice_end_time, interval}).
...
encode_pbtwistegglist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbtwistegglist(Record)
    when is_record(Record, pbtwistegglist) ->
    encode(pbtwistegglist, Record).

encode_pbtwistegg(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbtwistegg(Record)
    when is_record(Record, pbtwistegg) ->
    encode(pbtwistegg, Record).
...
encode_pbactivitylist(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbactivitylist(Record)
    when is_record(Record, pbactivitylist) ->
    encode(pbactivitylist, Record).

encode_pbactivity(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbactivity(Record)
    when is_record(Record, pbactivity) ->
    encode(pbactivity, Record).

encode(pbactivity, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbactivity, Record) ->
    [iolist(pbactivity, Record)
     | encode_extensions(Record)];
encode(pbactivitylist, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbactivitylist, Record) ->
    [iolist(pbactivitylist, Record)
     | encode_extensions(Record)];
...
encode(pbtwistegg, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbtwistegg, Record) ->
    [iolist(pbtwistegg, Record)
     | encode_extensions(Record)];
encode(pbtwistegglist, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbtwistegglist, Record) ->
    [iolist(pbtwistegglist, Record)
     | encode_extensions(Record)].
...
iolist(pbactivity, Record) ->
    [pack(1, optional,
	  with_default(Record#pbactivity.id, none), int32, []),
     pack(2, optional,
	  with_default(Record#pbactivity.activity_desc, none),
	  string, []),
     pack(3, optional,
	  with_default(Record#pbactivity.category, none), int32,
	  []),
     pack(4, optional,
	  with_default(Record#pbactivity.start_time, none), int32,
	  []),
     pack(5, optional,
	  with_default(Record#pbactivity.end_time, none), int32,
	  []),
     pack(6, optional,
	  with_default(Record#pbactivity.server_now, none), int32,
	  []),
     pack(7, optional,
	  with_default(Record#pbactivity.type, none), int32, []),
     pack(8, optional,
	  with_default(Record#pbactivity.notice, none), string,
	  []),
     pack(9, optional,
	  with_default(Record#pbactivity.notice_start_time, none),
	  int32, []),
     pack(10, optional,
	  with_default(Record#pbactivity.notice_end_time, none),
	  int32, []),
     pack(11, optional,
	  with_default(Record#pbactivity.interval, none), int32,
	  [])];
iolist(pbactivitylist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbactivitylist.activity, none),
	  pbactivity, []),
     pack(2, optional,
	  with_default(Record#pbactivitylist.mails_count, none),
	  int32, [])];
iolist(pbattribute, Record) ->
    [pack(1, optional,
...
iolist(pbtwistegg, Record) ->
    [pack(1, optional,
	  with_default(Record#pbtwistegg.type, none), int32, []),
     pack(2, optional,
	  with_default(Record#pbtwistegg.count, none), int32, []),
     pack(3, optional,
	  with_default(Record#pbtwistegg.add_att_flag, none),
	  int32, []),
     pack(4, repeated,
	  with_default(Record#pbtwistegg.goods_id, none), int32,
	  [])];
iolist(pbtwistegglist, Record) ->
    [pack(1, repeated,
	  with_default(Record#pbtwistegglist.twist_egg, none),
	  pbtwistegg, [])].
...
decode_pbtwistegglist(Bytes) when is_binary(Bytes) ->
    decode(pbtwistegglist, Bytes).

decode_pbtwistegg(Bytes) when is_binary(Bytes) ->
    decode(pbtwistegg, Bytes).
...
decode_pbactivitylist(Bytes) when is_binary(Bytes) ->
    decode(pbactivitylist, Bytes).

decode_pbactivity(Bytes) when is_binary(Bytes) ->
    decode(pbactivity, Bytes).

delimited_decode_pbactivity(Bytes) ->
    delimited_decode(pbactivity, Bytes).

delimited_decode_pbactivitylist(Bytes) ->
    delimited_decode(pbactivitylist, Bytes).
...
delimited_decode_pbtwistegg(Bytes) ->
    delimited_decode(pbtwistegg, Bytes).

delimited_decode_pbtwistegglist(Bytes) ->
    delimited_decode(pbtwistegglist, Bytes).
...
decode(pbactivity, Bytes) when is_binary(Bytes) ->
    Types = [{11, interval, int32, []},
	     {10, notice_end_time, int32, []},
	     {9, notice_start_time, int32, []},
	     {8, notice, string, []}, {7, type, int32, []},
	     {6, server_now, int32, []}, {5, end_time, int32, []},
	     {4, start_time, int32, []}, {3, category, int32, []},
	     {2, activity_desc, string, []}, {1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbactivity, Decoded);
decode(pbactivitylist, Bytes) when is_binary(Bytes) ->
    Types = [{2, mails_count, int32, []},
	     {1, activity, pbactivity, [is_record, repeated]}],
    Defaults = [{1, activity, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbactivitylist, Decoded);
...
decode(pbtwistegg, Bytes) when is_binary(Bytes) ->
    Types = [{4, goods_id, int32, [repeated]},
	     {3, add_att_flag, int32, []}, {2, count, int32, []},
	     {1, type, int32, []}],
    Defaults = [{4, goods_id, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbtwistegg, Decoded);
decode(pbtwistegglist, Bytes) when is_binary(Bytes) ->
    Types = [{1, twist_egg, pbtwistegg,
	      [is_record, repeated]}],
    Defaults = [{1, twist_egg, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbtwistegglist, Decoded).
...
to_record(pbactivity, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			  ...
			  #pbactivity{}, DecodedTuples),
    Record1;
to_record(pbactivitylist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbactivitylist),
...
to_record(pbskill, DecodedTuples) ->
    ...
    Record1;
to_record(pbtwistegg, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbtwistegg),
			  ...
			  #pbtwistegg{}, DecodedTuples),
    Record1;
to_record(pbtwistegglist, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbtwistegglist),
