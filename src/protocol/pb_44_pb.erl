...
-export([encode_pbskill/1, decode_pbskill/1,
	 ...
	 delimited_decode_pbresult/1, encode_pbplayercamp/1,
	 decode_pbplayercamp/1, delimited_decode_pbplayercamp/1,
	 ...
	 delimited_decode_pbcombatreward/1, encode_pbcamppos/1,
	 decode_pbcamppos/1, delimited_decode_pbcamppos/1,
	 encode_pbcamp/1, decode_pbcamp/1,
	 delimited_decode_pbcamp/1, encode_pbattribute/1,
...
-record(pbplayercamp,
	{player_id, using_camp, camp_list, update_timestamp}).
...
-record(pbcamppos, {pos, goods_uid}).

-record(pbcamp, {id, pos_list, update_timestamp}).
...
encode_pbplayercamp(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbplayercamp(Record)
    when is_record(Record, pbplayercamp) ->
    encode(pbplayercamp, Record).
...
encode_pbcamppos(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbcamppos(Record)
    when is_record(Record, pbcamppos) ->
    encode(pbcamppos, Record).

encode_pbcamp(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbcamp(Record) when is_record(Record, pbcamp) ->
    encode(pbcamp, Record).
...
encode(pbcamp, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbcamp, Record) ->
    [iolist(pbcamp, Record) | encode_extensions(Record)];
encode(pbcamppos, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbcamppos, Record) ->
    [iolist(pbcamppos, Record) | encode_extensions(Record)];
...
encode(pbidstring, Record) ->
    [iolist(pbidstring, Record)
     | encode_extensions(Record)];
encode(pbplayercamp, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbplayercamp, Record) ->
    [iolist(pbplayercamp, Record)
     | encode_extensions(Record)];
...
iolist(pbattribute, Record) ->
    [pack(1, optional,
     ...
     pack(18, optional,
	  ...
	  int32, [])];
iolist(pbcamp, Record) ->
    [pack(1, optional, with_default(Record#pbcamp.id, none),
	  int32, []),
     pack(2, repeated,
	  with_default(Record#pbcamp.pos_list, none), pbcamppos,
	  []),
     pack(3, optional,
	  with_default(Record#pbcamp.update_timestamp, none),
	  int32, [])];
iolist(pbcamppos, Record) ->
    [pack(1, optional,
	  with_default(Record#pbcamppos.pos, none), int32, []),
     pack(2, optional,
	  with_default(Record#pbcamppos.goods_uid, none), int64,
	  [])];
iolist(pbcombatreward, Record) ->
    [pack(1, optional,
...
iolist(pbplayercamp, Record) ->
    [pack(1, optional,
	  with_default(Record#pbplayercamp.player_id, none),
	  int64, []),
     pack(2, optional,
	  with_default(Record#pbplayercamp.using_camp, none),
	  int32, []),
     pack(3, repeated,
	  with_default(Record#pbplayercamp.camp_list, none),
	  pbcamp, []),
     pack(4, optional,
	  with_default(Record#pbplayercamp.update_timestamp,
		       none),
	  int32, [])];
...
decode_pbplayercamp(Bytes) when is_binary(Bytes) ->
    decode(pbplayercamp, Bytes).
...
decode_pbcamppos(Bytes) when is_binary(Bytes) ->
    decode(pbcamppos, Bytes).

decode_pbcamp(Bytes) when is_binary(Bytes) ->
    decode(pbcamp, Bytes).
...
delimited_decode_pbcamp(Bytes) ->
    delimited_decode(pbcamp, Bytes).

delimited_decode_pbcamppos(Bytes) ->
    delimited_decode(pbcamppos, Bytes).
...
delimited_decode_pbplayercamp(Bytes) ->
    delimited_decode(pbplayercamp, Bytes).
...
decode(pbcamp, Bytes) when is_binary(Bytes) ->
    Types = [{3, update_timestamp, int32, []},
	     {2, pos_list, pbcamppos, [is_record, repeated]},
	     {1, id, int32, []}],
    Defaults = [{2, pos_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcamp, Decoded);
decode(pbcamppos, Bytes) when is_binary(Bytes) ->
    Types = [{2, goods_uid, int64, []},
	     {1, pos, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbcamppos, Decoded);
...
decode(pbplayercamp, Bytes) when is_binary(Bytes) ->
    Types = [{4, update_timestamp, int32, []},
	     {3, camp_list, pbcamp, [is_record, repeated]},
	     {2, using_camp, int32, []}, {1, player_id, int64, []}],
    Defaults = [{3, camp_list, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbplayercamp, Decoded);
...
to_record(pbattribute, DecodedTuples) ->
    ...
    Record1;
to_record(pbcamp, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, pbcamp),
						   Record, Name, Val)
			  end,
			  #pbcamp{}, DecodedTuples),
    Record1;
to_record(pbcamppos, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbcamppos),
...
to_record(pbidstring, DecodedTuples) ->
    ...
    Record1;
to_record(pbplayercamp, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbplayercamp),
