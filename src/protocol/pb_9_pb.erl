-file("src/pb_9_pb.erl", 1).

-module(pb_9_pb).

-export([encode_pbnull/1, decode_pbnull/1,
	 delimited_decode_pbnull/1, encode_pbskill/1,
	 ...
	 encode_pbnewiteminfo/1, decode_pbnewiteminfo/1,
	 delimited_decode_pbnewiteminfo/1, encode_pbidstring/1,
	 ...
	 delimited_decode_pbfriend/1, encode_pberror/1,
	 decode_pberror/1, delimited_decode_pberror/1,
...
-record(pberror, {error_code, args}).
...
encode_pbnull(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbnull(Record) when is_record(Record, pbnull) ->
    encode(pbnull, Record).
...
encode_pbnewiteminfo(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pbnewiteminfo(Record)
    when is_record(Record, pbnewiteminfo) ->
    encode(pbnewiteminfo, Record).
...
encode_pberror(Records) when is_list(Records) ->
    delimited_encode(Records);
encode_pberror(Record)
    when is_record(Record, pberror) ->
    encode(pberror, Record).
...
encode(pbcombatreward, Record) ->
    [iolist(pbcombatreward, Record)
     | encode_extensions(Record)];
encode(pberror, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pberror, Record) ->
    [iolist(pberror, Record) | encode_extensions(Record)];
...
encode(pbidstring, Record) ->
    [iolist(pbidstring, Record)
     | encode_extensions(Record)];
encode(pbnewiteminfo, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbnewiteminfo, Record) ->
    [iolist(pbnewiteminfo, Record)
     | encode_extensions(Record)];
...
encode(pbnull, Records) when is_list(Records) ->
    delimited_encode(Records);
encode(pbnull, Record) ->
    [iolist(pbnull, Record) | encode_extensions(Record)].
...
iolist(pbcombatreward, Record) ->
    [pack(1, optional,
     ...
     pack(8, repeated,
	  ...
	  int64, [])];
iolist(pberror, Record) ->
    [pack(1, optional,
	  with_default(Record#pberror.error_code, none), int32,
	  []),
     pack(2, repeated,
	  with_default(Record#pberror.args, none), string, [])];
...
iolist(pbnewiteminfo, Record) ->
    [pack(1, optional,
	  with_default(Record#pbnewiteminfo.base_id, none), int32,
	  [])];
...
iolist(pbnull, Record) ->
    [pack(1, optional, with_default(Record#pbnull.id, none),
	  int32, [])].
...
decode_pbnull(Bytes) when is_binary(Bytes) ->
    decode(pbnull, Bytes).
...
decode_pbnewiteminfo(Bytes) when is_binary(Bytes) ->
    decode(pbnewiteminfo, Bytes).
...
decode_pberror(Bytes) when is_binary(Bytes) ->
    decode(pberror, Bytes).
...
delimited_decode_pberror(Bytes) ->
    delimited_decode(pberror, Bytes).
...
delimited_decode_pbnewiteminfo(Bytes) ->
    delimited_decode(pbnewiteminfo, Bytes).
...
delimited_decode_pbnull(Bytes) ->
    delimited_decode(pbnull, Bytes).
...
decode(pberror, Bytes) when is_binary(Bytes) ->
    Types = [{2, args, string, [repeated]},
	     {1, error_code, int32, []}],
    Defaults = [{2, args, []}],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pberror, Decoded);
...
decode(pbnewiteminfo, Bytes) when is_binary(Bytes) ->
    Types = [{1, base_id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbnewiteminfo, Decoded);
...
decode(pbnull, Bytes) when is_binary(Bytes) ->
    Types = [{1, id, int32, []}],
    Defaults = [],
    Decoded = decode(Bytes, Types, Defaults),
    to_record(pbnull, Decoded).
...
to_record(pbcombatreward, DecodedTuples) ->
    ...
    Record1;
to_record(pberror, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, pberror),
						   Record, Name, Val)
			  end,
			  #pberror{}, DecodedTuples),
    Record1;
...
to_record(pbidstring, DecodedTuples) ->
    ...
    Record1;
to_record(pbnewiteminfo, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields,
							       pbnewiteminfo),
...
to_record(pbskill, DecodedTuples) ->
    ...
    Record1;
to_record(pbnull, DecodedTuples) ->
    Record1 = lists:foldr(fun ({_FNum, Name, Val},
			       Record) ->
				  set_record_field(record_info(fields, pbnull),
						   Record, Name, Val)
			  end,
			  #pbnull{}, DecodedTuples),
    Record1.
