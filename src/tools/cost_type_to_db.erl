...
start() ->
    ...
    emysql:execute(db_base, "DELETE FROM base_money_cost"),
    Res = emysql:execute(db_base, SQL),
    io:format("~p~n", [Res]),
    ok.

header_files() ->
    filelib:wildcard(filename:join([app_misc:server_root(), "include/define_money_cost.hrl"])).

insert_base_cost_type_list(List) ->	
    BaseSQL = "INSERT INTO `base_money_cost` (`id`, `macro`, `name`) VALUES ",
