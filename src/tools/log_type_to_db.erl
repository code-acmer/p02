...
start() ->
    ...
    emysql:execute(db_base, "DELETE FROM base_log_type"),
    Res = emysql:execute(db_base, SQL),
    io:format("~p~n", [Res]),
    ok.

header_files() ->
    filelib:wildcard(filename:join([app_misc:server_root(), "include/define_log_type.hrl"])).

insert_base_cost_type_list(List) ->	
    BaseSQL = "INSERT INTO `base_log_type` (`id`, `macro`, `name`) VALUES ",
