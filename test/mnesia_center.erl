...
link_nodes() ->
    mnesia:start(),
    net_adm:ping('test_mnesia_node_172@192.168.1.172'),
    ...
    [
     {test_disc_only,
      [
       {attributes, record_info(fields, test_disc_only)},
       %% {index, [#test_disc_only.key]},
       {frag_properties, [{n_fragments, 4}, {n_disc_only_copies, 1}, {node_pool,[node()]}]},
       {disc_only_copies,[node()]}
      ]},
     %% 物品表
     {test_disc,
      [
       {attributes, record_info(fields, test_disc)},
       %% {index, [#test_disc.key]},
       {frag_properties, [{n_fragments, 4}, {n_disc_copies, 1}, {node_pool,[node()]}]},
       {disc_copies,[node()]}
      ]
     },
     {test_ram,
      [
       {attributes, record_info(fields, test_ram)},
       {frag_properties, [{n_fragments, 4}, {n_ram_copies, 1}, {node_pool,[node()]}]},
       %% {index, [#test_ram.key]},
       {ram_copies,[node()]}
      ]}     
    ].
    ...
    [
     {test_disc_only,
      [
       {attributes, record_info(fields, test_disc_only)},
       %% {index, [#test_disc_only.key]},
       {frag_properties, [{n_fragments, 64}, {n_disc_only_copies, 1}, {node_pool,[node()]}]},
       {disc_only_copies,[node()]}
      ]},
     %% 物品表
     {test_disc,
      [
       {attributes, record_info(fields, test_disc)},
       %% {index, [#test_disc.key]},
       {frag_properties, [{n_fragments, 64}, {n_disc_copies, 1}, {node_pool,[node()]}]},
       {disc_copies,[node()]}
      ]
     },
     {test_ram,
      [
       {attributes, record_info(fields, test_ram)},
       {frag_properties, [{n_fragments, 64}, {n_ram_copies, 1}, {node_pool,[node()]}]},
       %% {index, [#test_ram.key]},
       {ram_copies,[node()]}
      ]}     
    ].
...
disc_select() ->
    ...
    Pids = lists:map(fun(_I) ->
                             erlang:spawn_monitor(fun() ->
                                                          lists:foreach(fun(Key) ->
                                                                                hdb:select(test_disc, [{key,"<",Key}],[],{limit, 1})
                                                                        end, lists:seq(1,100))
                                                  end)
                     end, lists:seq(1, 1000)),
    [receive
...
disc_select_with_orderby() ->
    Before2 = os:timestamp(),  
    Pids = lists:map(fun(_I) ->
                             erlang:spawn_monitor(fun() ->
                                                          lists:foreach(fun(Key) ->
                                                                                hdb:select(test_disc, [{key,"<", Key}],[{key,1}],{limit, 1})
                                                                        end, lists:seq(1,100))
                                                  end)
                     end, lists:seq(1, 100)),
    [receive
